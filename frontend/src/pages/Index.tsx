...
import { CheckCircle2, TrendingUp, User, Briefcase, Globe, Calendar, Sparkles, Bot } from "lucide-react";
...
// AI PILLAR Components
import { CoPilotPanel } from "@/components/copilot/CoPilotPanel";
import { QuestionBankPanel } from "@/components/questions/QuestionBankPanel";
import { FeedbackAlertsPanel } from "@/components/feedback/FeedbackAlertsPanel";
import { RAGAssistantPanel } from "@/components/assistant/RAGAssistantPanel";
import { DocumentUploadPanel } from "@/components/knowledge/DocumentUploadPanel";
import { CVUploadPanel } from "@/components/candidates/CVUploadPanel";
import { CandidateProfilesPanel } from "@/components/candidates/CandidateProfilesPanel";
...
const Index = () => {
  ...
  const interviewId = id ? parseInt(id) : 1; // Default to 1 for demo
  ...
  const [linkedCandidateId, setLinkedCandidateId] = useState<number | undefined>();
  ...
  const handleRoomCreated = useCallback(async (newRoomName: string, candidateId?: number, newInterviewId?: number) => {
    console.log('[Index] Room created:', newRoomName, 'Candidate ID:', candidateId, 'Interview ID:', newInterviewId);
    // Update room name - the hook will automatically reconnect
    setRoomName(newRoomName);

    // Store the candidate ID if provided
    if (candidateId) {
      setLinkedCandidateId(candidateId);
      console.log('[Index] Candidate linked to interview:', candidateId);
    }

    // Note: Interview record is automatically created by the backend with candidate_id
    // No need to call linkInterviewToCandidate anymore
  }, []);
  ...
  const handleEnd = async () => {
    if (confirm('Are you sure you want to end this interview session?')) {
      ...
      // Process interview completion to update candidate profile
      if (linkedCandidateId && interviewId) {
        try {
          console.log('[Index] Processing interview completion for candidate:', linkedCandidateId);
          await api.processInterviewCompletion(interviewId);
          console.log('[Index] Interview completion processed successfully');
          alert('Interview notes have been added to the candidate profile!');
        } catch (error) {
          console.error('[Index] Failed to process interview completion:', error);
          alert('Failed to process interview completion. Please check the console for errors.');
        }
      } else {
        console.warn('[Index] No candidate linked to this interview. linkedCandidateId:', linkedCandidateId, 'interviewId:', interviewId);
        console.warn('[Index] Interview notes will NOT be saved to any candidate profile.');
        if (!linkedCandidateId) {
          alert('No candidate was selected for this interview. Interview notes will not be saved.\n\nNext time, select a candidate from the dropdown before creating the room.');
        }
      }
    }
  };
    ...
    <div className="min-h-screen flex flex-col bg-bg-primary">
      ...
      <div className="flex-1 overflow-hidden">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full flex flex-col">
          ...
          <TabsContent value="ai-tools" className="flex-1 m-0 overflow-auto p-6">
            <div className="max-w-7xl mx-auto space-y-6">
              <div>
                <h2 className="text-2xl font-bold mb-2">AI-Powered Interview Tools</h2>
                <p className="text-muted-foreground">
                  Real-time assistance, intelligent suggestions, and expert guidance
                </p>
              </div>

              <div className="grid grid-cols-12 gap-6">
                {/* Left Column - Co-Pilot & Question Bank */}
                <div className="col-span-6 space-y-6">
                  <CoPilotPanel interviewId={interviewId} />
                  <QuestionBankPanel interviewId={interviewId} jobTitle={INTERVIEWER_PROFILE.role} />
                </div>

                {/* Right Column - Feedback & Assistant */}
                <div className="col-span-6 space-y-6">
                  <FeedbackAlertsPanel interviewId={interviewId} />
                  <div className="h-[600px]">
                    <RAGAssistantPanel
                      interviewId={interviewId}
                      interviewContext={{
                        job_title: INTERVIEWER_PROFILE.role,
                        candidate_name: "Candidate",
                        duration_minutes: duration / 60,
                        questions_asked: transcriptFeedData.length,
                        competencies_covered: evaluations.flatMap(e => e.key_topics),
                        coverage_gaps: []
                      }}
                    />
                  </div>
              ...
              {/* Full Width - CV Upload for Candidate Profiles */}
              <div className="mt-6">
                <CVUploadPanel />
              </div>

              {/* Full Width - Document Upload for Knowledge Base */}
              <div className="mt-6">
                <DocumentUploadPanel />
              </div>
            </div>
          ...
          <TabsContent value="candidates" className="flex-1 m-0 overflow-hidden p-6">
