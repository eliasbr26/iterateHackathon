...
interface VideoAreaProps {
  onRoomCreated?: (roomName: string, candidateId?: number, interviewId?: number) => void;
}
...
const VideoAreaComponent = ({ onRoomCreated }: VideoAreaProps) => {
  ...
  const handleCreateRoom = async () => {
    setIsJoining(true);
    try {
      const response = await api.createRoom(undefined, 10, selectedCandidateId);
      ...
      // Notify parent component about room creation with interview ID and candidate ID
      onRoomCreated?.(response.name, selectedCandidateId, response.interview_id);

      toast({
        title: 'Room Created',
        description: selectedCandidateId
          ? `Successfully created and joined room: ${response.name} for candidate (Interview #${response.interview_id})`
          : `Successfully created and joined room: ${response.name} (Interview #${response.interview_id})`,
      });
    } catch (err) {
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-6">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="candidate">Select Candidate (Optional)</Label>
            <Select
              value={selectedCandidateId?.toString() || 'none'}
              onValueChange={(value) => setSelectedCandidateId(value === 'none' ? undefined : parseInt(value))}
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="none">No candidate selected</SelectItem>
