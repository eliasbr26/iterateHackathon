...
class ClaudeBatchAnalyzer:
    ...
    def _parse_response(self, response_text: str) -> List[str]:
        ...
        # The pattern r"\[([A-Z_]+)\]" looks for:
        # \[     - a literal opening bracket
        # ([A-Z_]+) - a capturing group (the part we want)
        #             matching one or more uppercase letters or underscores
        # \]     - a literal closing bracket
        
        tags = re.findall(r"\[([A-Z_]+)\]", response_text)
        
        # The result 'tags' is a list like ['CV_TECHNIQUES', 'STATIONARITY', 'EXTRA']
        # We add the brackets back for consistency.
        return [f"[{tag}]" for tag in tags]

    def analyze_text(self, transcript: str) -> List[str]:
        """
...
if not API_KEY or API_KEY == "votre_cle_api_anthropic_ici":
    ...
    for sentence in challenging_conversation_flow:
        if speaker == "Recruiter" and sentence["speaker"] == "Candidate":
            ...
            covered_theme = engine.analyze_text(current_text)
            output.append({'text': current_text, 'covered_theme': covered_theme})
    ...
    print("\n--- FIN DE L'ENTRETIEN ---")
    print("Catégories classifiées :")
    for out in output:
        print(out['text'])
        print(out['covered_theme'])